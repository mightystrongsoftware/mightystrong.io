...
Building a language app when you don't speak the language means you need someone who does. My wife doesn't speak Slovenian at all. The language didn't make it down through her family. My own exposure? Serving Slovenian mass at St. Mary's Collinwood as a kid. That's the entire household Slovenian knowledge base.

So the content expert was also an AI agent. A non-Slovenian developer, using an AI agent to generate Slovenian language courses, building an app to teach a language that nobody in the household actually speaks. Peak 2026.

But it seems to work, and the reason comes down to **guardrails**. The content agent couldn't just freestyle. It needed to produce lessons in a structure the frontend could render. I defined that contract up front: what lesson types the app supported, what fields each exercise needed, how progression would work. The agent authored courses within those constraints, and a separate layer validated everything before it hit the app.
...
The development itself went beyond typical agentic engineering into something more like exploratory product development. It wasn't just "agents help me code this." It was more of a trickle down: ideas to refinement to new ideas to further refinement, and only then to development. The agents were shaping what got built, not just executing on a spec.
...
- **Content Expert**: A master of Slovenian and the pedagogy ([if you're like me and had to look that up](https://en.wikipedia.org/wiki/Pedagogy)) of teaching it. This agent understood the right progressions, when to introduce grammar concepts, how to scaffold vocabulary. It authored the actual course content within the schema constraints.
- **Content Reviewer**: This one came later out of necessity. Its job was to audit the course content looking for concepts introduced too early or too frequently. Introducing something early is fine, it makes the learner stretch a little. But too early *and* too often makes it feel daunting rather than challenging, and that's where people quit.
...
It's live now at [naprej.cloud](https://naprej.cloud/), no authentication required for now. If you have any thoughts, reach out at [evan@mightystrong.io](mailto:evan@mightystrong.io).

Still not Slovenian. Still can't order coffee in Ljubljana without pointing at the menu. But now there's an app for that.
